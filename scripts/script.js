...
var START_DATE = new Date('January 7, 2019'); //The start day of the school year. This should be a weekday.
...
function calculateScheduleRotationID(date) {
    var daysDifference = Math.round((date.getTime() - START_DATE.getTime()) / MILLIS_PER_DAY);
    //Factor out weekends
...
function createPeriod(parent, name, start, end, date, showTime) {
    ...
    if (length > 0) {
        ...
        if(LINKS[name]) {
            ...
            linkWrapper.href = LINKS[name].split("$DATE$").join(date.getDate()).split("$DAY$").join(date.getDay() + 1).split("$FULLYEAR$").join(date.getFullYear()).split("$MONTH$").join(date.getMonth() + 1).split("$DAYNAME$").join(days[date.getDay()]).split("$DAYNAMELOWER$").join(days[date.getDay()].toLowerCase());
